...
# Blueprint setup
plex_oauth_bp = Blueprint('plex_oauth', __name__, url_prefix='/api/plex/oauth')
...
# Decorator for routes requiring valid Plex token
def require_valid_plex_token(f):
    """Decorator to ensure valid Plex token."""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # Get current user (adapt to Bazarr's auth system)
...
# Routes
@plex_oauth_bp.route('/pin', methods=['POST'])
def create_pin():
    ...
    try:
        ...
        headers = {
            ...
            'X-Plex-Version': current_app.version if hasattr(current_app, 'version') else '1.0',
        ...
        # Store PIN in cache/session with expiration
        # (implement cache_pin based on your caching system)
        from bazarr.app.cache import cache_pin
...
@plex_oauth_bp.route('/pin/<pin_id>/check', methods=['GET'])
def check_pin(pin_id):
    """Check PIN status."""
    try:
        # Get cached PIN (implement get_cached_pin based on your caching system)
        from bazarr.app.cache import get_cached_pin, delete_cached_pin
        ...
        if pin_data.get('authToken'):
            ...
            user.plex_user_id = str(user_data.get('id'))
            database.commit()
...
@plex_oauth_bp.route('/validate', methods=['GET'])
...
@plex_oauth_bp.route('/servers', methods=['GET'])
@require_valid_plex_token
def get_servers():
    """Get Plex servers."""
    try:
...
@plex_oauth_bp.route('/logout', methods=['POST'])
def logout():
...
# Error handlers
@plex_oauth_bp.errorhandler(PlexAuthError)
...
@plex_oauth_bp.errorhandler(Exception)
def handle_generic_error(e):
    current_app.logger.error(f"Unhandled error in Plex OAuth: {str(e)}")
    return jsonify({
